...
class PublishToBlockchain:
    ...
    def __init__(self):
        ...
        def get_file_cb(req):
            ...
            del self.res
            self.got_file = False
            files = os.listdir('/media/sf_ssh_user/stl')
            while len(files) > 0:
                rospy.loginfo("in process get_file_cb: " + files[0])
                rospy.sleep(1)
                files = os.listdir('/media/sf_ssh_user/stl')
        ...
        def callback(data):
            ...
            rospy.loginfo("copying {} to {}".format(self.path_to_file, '/media/sf_ssh_user/stl'))
            self.res = copy2(self.path_to_file, '/media/sf_ssh_user/stl')
            ...
            files = os.listdir('/media/sf_ssh_user/stl')
            while len(files) > 0:
                rospy.loginfo("in process: " + files[0])
                rospy.sleep(1)
                files = os.listdir('/media/sf_ssh_user/stl')
